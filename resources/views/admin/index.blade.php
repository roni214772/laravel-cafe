...
<div class="topbar">
  <div class="topbar-brand">
    ...
    @if($pendingCount > 0)
      <span class="badge-notify">{{ $pendingCount }} bekliyor</span>
    @endif
  </div>
  <div class="topbar-spacer"></div>
  <div class="topbar-right">
    <a href="{{ route('adisyon.index') }}" class="tb-btn">← Adisyon</a>
    <a href="{{ route('admin.login') }}" class="tb-btn" title="Admin giriş sayfası">🔑 Giriş URL</a>
...
<div class="page">
  ...
  <div class="card" style="margin-bottom:18px">
    ...
    <div style="padding:18px 20px">
      <form method="POST" action="{{ route('admin.update-prices') }}" style="display:flex;gap:14px;flex-wrap:wrap;align-items:flex-end">
        @csrf
        <div style="display:flex;flex-direction:column;gap:5px">
          ...
          <input type="number" name="price_monthly" value="{{ $priceMonthly }}" step="0.01" min="1"
  ...
  <div class="card" style="margin-bottom:18px">
    ...
    <div style="padding:18px 20px">
      <form method="POST" action="{{ route('admin.update-bank') }}" style="display:flex;gap:14px;flex-wrap:wrap;align-items:flex-end">
        @csrf
        <div style="display:flex;flex-direction:column;gap:5px">
          <label style="font-size:.67rem;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Banka Adı</label>
          <input type="text" name="bank_name" value="{{ $bankName }}" placeholder="Ziraat Bankası"
        ...
        <div style="display:flex;flex-direction:column;gap:5px">
          ...
          <input type="text" name="bank_iban" value="{{ $bankIban }}" placeholder="TR00 0000 0000 0000 0000 0000 00"
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            @elseif($st === 'pending')
              <span class="badge orange">⏳ Bekliyor — {{ $tp === 'yearly' ? 'Yıllık' : 'Aylık' }}</span>
            @elseif($st === 'active')
              <span class="badge green">✓ Aktif — {{ $tp === 'yearly' ? 'Yıllık' : 'Aylık' }}</span>
            @elseif($st === 'rejected')
            ...
            @if($user->email !== auth()->user()->email)
              <div class="td-actions">
                @if($user->subscription_status === 'pending')
                  ...
                  <form method="POST" action="{{ route('admin.reject', $user) }}">
                    @csrf
                ...
                <form method="POST" action="{{ route('admin.delete-user', $user) }}"
                      onsubmit="return confirm('{{ addslashes($user->name) }} silinsin mi?')">
                  @csrf @method('DELETE')
      ...
      <div class="mob-item">
        <div class="mob-item-top">
          ...
          <div style="min-width:0">
            ...
            <div class="mob-email">{{ $user->email }}</div>
          </div>
        </div>
        <div class="mob-meta">
