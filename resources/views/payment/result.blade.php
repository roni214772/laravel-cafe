...
<title>Ödeme Sonucu  Kafe POS</title>
...
<div class="card">
  @if(session('payment_pending'))
    ...
    <a href="{{ route('subscription.pending') }}" class="btn primary">Durumu Takip Et</a>
  @else
    <span class="icon"></span>
    <h1 style="color:#ef4444">Bir sorun oluştu</h1>
    <p>{{ session('error_msg') ?: 'Lütfen tekrar deneyin.' }}</p>
    <a href="{{ route('subscription.select') }}" class="btn primary">Tekrar Dene</a>
primary">Tekrar Dene</a>
    <a href="{{ route('adisyon.index') }}" class="btn outline">Vazgeç</a>
  @endif
</div>
</body>
</html>
