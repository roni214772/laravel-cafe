...
@php
  ...
  $ci++;
@endphp
      <div class="cat-card {{$cls}}" onclick="openCat('{{$eid}}')">
        <div class="cat-img-wrap">
          @if($imgUrl)
          <img src="{{$imgUrl}}" alt="{{$cat}}" loading="lazy">
          @else
          <div class="cat-img-empty"></div>
          @endif
        </div>
        <div class="cat-label">
          <div class="cn">{{$cat ?: 'Genel'}}</div>
        </div>
      </div>
@endforeach
    </div>
...
@foreach($products->sortKeys() as $cat => $items)
@php $eid = 'sec_'.preg_replace('/[^a-zA-Z0-9]/','_',$cat); @endphp
    <div id="{{$eid}}" style="display:none">
@foreach($items as $p)
      <div class="product-card" onclick="openModal({{$p->id}})">
        <div class="pinfo">
          <div class="pname">{{$p->name}}</div>
          @if($p->description)<div class="pdesc">{{$p->description}}</div>@endif
          <div class="pprice">{{number_format($p->price,2,',','.')}} &#8378;</div>
        </div>
        <div class="pimg-wrap">
          @if($p->image_url)
          <img src="{{$p->image_url}}" alt="{{$p->name}}" loading="lazy" onerror="this.parentNode.innerHTML='<span class=pimg-empty>&#9749;</span>'">
          @else
...
@endphp
var ALL_PRODUCTS = {!! json_encode($allProductsJson, JSON_HEX_TAG|JSON_HEX_QUOT|JSON_UNESCAPED_UNICODE) !!};
var PRODUCTS = {!! $productsJson !!};
var CAT_MAP  = {!! $catMapJson !!};
var currentSec = null;
(function(){
