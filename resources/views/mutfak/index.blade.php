...
<div class="main">
  <div class="orders-grid" id="kitchenGrid">
    @forelse($orders as $order)
      @php $hasPending = collect($order['items'])->contains('status', 'pending'); @endphp
      <div class="kcard {{ $hasPending ? 'urgent' : 'done' }}" data-order-id="{{ $order['id'] }}">
        <div class="kcard-head">
          <div class="kcard-head-left">
            <div class="table-label">Masa</div>
            <div class="table-name">{{ $order['name'] }}</div>
            <div class="table-time">
              <span class="time-dot"></span>
              {{ $order['opened'] ?? now()->format('H:i') }}
            </div>
          </div>
          <div class="kcard-head-right">
            <div class="item-badge">{{ count($order['items']) }} kalem</div>
            <div class="order-num">#{{ $order['id'] }}</div>
          </div>
        </div>
        <div class="kcard-items">
          @foreach($order['items'] as $item)
            @php $q=$item['qty']; $qL=fmod((float)$q,1.0)==0?(int)$q:number_format((float)$q,1); @endphp
            <div class="item-row {{ $item['status'] === 'ready' ? 'ready-row' : '' }}">
              <div class="item-qty-box">{{ $qL }}</div>
              ...
              <div class="item-status-badge {{ $item['status'] === 'ready' ? 'ready' : 'pending' }}">
                {{ $item['status'] === 'ready' ? '✅ Hazır' : '⏳ Bekliyor' }}
              </div>
            </div>
          @endforeach
        </div>
        <div class="kcard-foot">
          @if($hasPending)
            <button class="btn-hazir hazir-btn" data-order-id="{{ $order['id'] }}">
...
const POLL_URL = '{{ route("mutfak.poll") }}';
const READY    = '{{ route("mutfak.mark-ready") }}';
