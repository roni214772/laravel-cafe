...
<div class="wrap">
  ...
  <h2>Merhaba, <?php echo e(auth()->user()->name); ?>! 👋</h2>
  <p class="sub" style="margin-bottom:18px">
    <?php if(auth()->user()->isSubscriptionActive()): ?>
  ...
  <?php if($isActive && $expiresAt): ?>
    <div class="status-card <?php echo e($daysLeft <= 3 ? 'expiring' : ($daysLeft <= 10 ? 'pending' : 'active')); ?>">
      <div class="status-icon">
        <?php echo e($daysLeft <= 3 ? '⚠️' : ($daysLeft <= 10 ? '⏰' : '✅')); ?>

      </div>
      <div class="status-info">
        <div class="status-title <?php echo e($colorClass); ?>">
          <?php if($daysLeft <= 3): ?>
        ...
        <div class="status-desc">
          Bitiş tarihi: <strong style="color:var(--text)"><?php echo e($expiresAt->format('d.m.Y')); ?></strong>
          &nbsp;•&nbsp;
          <?php echo e($user->subscription_type === 'yearly' ? 'Yıllık' : 'Aylık'); ?> plan
        </div>
        <div class="progress-wrap">
          <div class="progress-bar <?php echo e($colorClass); ?>" style="width:<?php echo e($progressPct); ?>%"></div>
        </div>
      </div>
      <div class="days-badge">
        <div class="days-num" style="color:var(--<?php echo e($colorClass === 'green' ? 'green' : ($colorClass === 'orange' ? 'orange' : 'red')); ?>)"><?php echo e($daysLeft); ?></div>
  ...
  <?php if($isPending): ?>
    <div class="status-card pending" style="margin-bottom:24px">
      ...
      <div class="status-info">
        ...
        <div class="status-desc">
          <?php echo e($user->subscription_type === 'yearly' ? 'Yıllık' : 'Aylık'); ?> plan talebi admin onayı bekliyor.<br>
          <?php if($expiresAt && $expiresAt->isFuture()): ?>
            Mevcut aboneliğiniz <strong style="color:var(--text)"><?php echo e($expiresAt->format('d.m.Y')); ?></strong> tarihine kadar geçerli, erişiminiz devam ediyor.
          <?php else: ?>
  ...
  <form method="GET" action="<?php echo e(route('payment.checkout')); ?>" id="planForm">
    <div class="plans">
      <label class="plan" id="planMonthly" onclick="selectPlan('monthly')">
        ...
        <div class="plan-price">₺<?php echo e(number_format((float) \App\Models\Setting::get('price_monthly', '299.00'), 0, ',', '.')); ?><span>/ay</span></div>
      ...
      <label class="plan" id="planQuarterly" onclick="selectPlan('quarterly')">
        ...
        <div class="plan-price">₺<?php echo e(number_format((float) \App\Models\Setting::get('price_quarterly', '799.00'), 0, ',', '.')); ?><span>/3ay</span></div>
      ...
      <label class="plan" id="planSemi" onclick="selectPlan('semi_yearly')">
        ...
        <div class="plan-price">₺<?php echo e(number_format((float) \App\Models\Setting::get('price_semi_yearly', '1490.00'), 0, ',', '.')); ?><span>/6ay</span></div>
      ...
      <label class="plan" id="planYearly" onclick="selectPlan('yearly')">
        ...
        <div class="plan-price">₺<?php echo e(number_format((float) \App\Models\Setting::get('price_yearly', '2799.00'), 0, ',', '.')); ?><span>/yıl</span></div>
  ...
  <div class="footer">
    <?php if(auth()->user()->isSubscriptionActive()): ?>
      <a href="<?php echo e(route('adisyon.index')); ?>">← Adisyon'a Dön</a>
    <?php else: ?>
      <a href="<?php echo e(route('logout')); ?>" onclick="event.preventDefault();document.getElementById('logoutForm').submit()">Çıkış Yap</a>
    <?php endif; ?>
  </div>
  <form id="logoutForm" method="POST" action="<?php echo e(route('logout')); ?>" style="display:none"><?php echo csrf_field(); ?></form>
</div>
...
<?php /**PATH C:\Users\brusk\OneDrive\Masaüstü\eto\laravel-cafe\resources\views/subscription/select.blade.php ENDPATH**/ ?>